...
func (r *FruitRepository) Create(ctx context.Context, f models.Fruit) (int, error) {
  query := `INSERT INTO fruits (name, brand, price_per_kg, stock_kg) 
            VALUES ($1, $2, $3, $4) RETURNING id`
  var id int
  err := r.db.QueryRowContext(ctx, query, f.Name, f.Brand, f.PricePerKg, f.StockKg).Scan(&id)
  return id, err
}
